...
class Tile(Widget):
    ...
    (x1,x2,x3,y1,y2,y3,y4) = (NumericProperty(), NumericProperty(), NumericProperty(),
                              NumericProperty(), NumericProperty(), NumericProperty(), NumericProperty())
##    for v in ('x1','x2','x3','y1','y2','y3','y4'):
##        exec(v + ' = NumericProperty()')
    points = ReferenceListProperty(x1,y1, x2,y1, x2,y2, x1,y2)
    def __init__(self, parent, x, y, init_color, **kwargs):
        ...
        self.x3, self.y3 = self.x+self.width/2, self.y+15
        self.y4 = self.y+self.height+15
##        self.points=[self.x,self.y, self.x+self.width,self.y, self.x+self.width,self.y+self.height, self.x,self.y+self.height]

        
        self.points = [self.x1,self.y1, self.x2,self.y, self.x2,self.y2, self.x,self.y2]
...
##                self.parent.tiles[tile].flip(self.parent.tiles[tile])
    ...
    def flip(self, instance):
        ...
        anim&= Animation(points=(self.x3,self.y3, self.x3,self.y1, self.x3, self.y2, self.x3,self.y4), d=.05)
